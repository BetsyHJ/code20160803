...
def Print(y, params_file, nerual_num):
    file = open(params_file, 'w')
    params = y
    file.write(' '.join(map(lambda x: str(x), nerual_num))+"\n")
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=100,
   ...
   """
    datasets, _ = load_data("train_feature1")
    ...
    nerual_num = [717, 512, 512, 128, 1000]
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in range(n_train_batches):
            ...
            if patience <= iter:
                done_looping = True
                break
        Print(classifier.params, "result/result_WB_epoch"+str(epoch), nerual_num)
