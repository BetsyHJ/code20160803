   ...
   """
	...
	#user = e_user; size = len(user)
        for iter in range(iteration): 
            #right_each = 0
            for i in range(batch_num):
                u_input, e1_input, e2_input = load_data(size_e, e_train, size_u, u_trains[i], u_c)
                sum_right = 0
            ...
            if (iter+1) % 10 == 0:
                Print(Params, "./result_all/result2_reg3_WB_nega"+str(sys.argv[1])+"_iter"+str(iter+1),[717] + nerual_num_U, [512])
