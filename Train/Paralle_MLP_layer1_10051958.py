...
def create_matrix(num_train, flag):
    ...
    return temp  
def load_data(size_e, e_train, size_u, u_train, u_c, borrow=True):
    negative_set = 10
    user_neigh_nega = [] #[u, e1, e2] index
        ...
        for iter in range(iteration): 
            ...
            for i in u_train:
                ...
                for j1 in range(len(c_list)):
                    c_Nega = []
		    neighbor = c_list[j1]
		    if neighbor not in e_train:
                        continue
                    #neigh_nega[c_list[j1]] = []
			...
			c_Nega.append(t)
                        temp_id = e_user[t]
                        user_neigh_nega.append([i, neighbor, temp_id])
    return user_neigh_nega
...
def prepare_data(user_neigh_nega, u_train, e_train):
    u_input = []
    e1_input = []
    e2_input = []
    for group in user_neigh_nega:
        u_input.append(u_train[group[0]])
        e1_input.append(e_train[group[1]])
        e2_input.append(e_train[group[2]])
    u_input = np.array(u_input, dtype = np.float32)#theano.shared(u_input)
    e1_input = np.array(e1_input, dtype = np.float32)#theano.shared(c1_input)
    e2_input = np.array(e2_input, dtype = np.float32)#theano.shared(c2_input)
    return u_input, e1_input, e2_input
        
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             dataset='mnist.pkl.gz', batch_size=64, n_hidden=500):
    """
   ...
   """
    ...
    wb_Embedfile = "train_feature2"#"../jd_wfeature"
    jd_wb_linkfile = "network"#"../../net_work10w"
    size_e, e_train = get_vector(jd_Embfile, 1)
    size_u, u_train = get_vector(wb_Embedfile, 0)
    u_c = get_net_control(jd_wb_linkfile, 30) # control the number of neighbor, set number 30
    user_neigh_nega = load_data(size_e, e_train, size_u, u_train, u_c)   
    
    batch_num = 15
    batch_size2 = len(user_neigh_nega) / batch_num
    print batch_size2, len(u_train)
    #u_trains = cut_data(u_train, batch_num)
    
    data1, data2, data3 = prepare_data(user_neigh_nega[:batch_size2], u_train, e_train)
    u_input = theano.shared(data1, borrow=True)
    e1_input = theano.shared(data2, borrow=True)
    e2_input = theano.shared(data3, borrow=True)
    ...
    for cross_i in range(1):#5 cross check
 	#params_file = "./result_all/result_WB_cross_512_nega1_init"#+str(cross_i)
        ...
        for iter in range(iteration): 
            #right_each = 0
            random.shuffle(user_neigh_nega)
            for i in range(batch_num):
                data1, data2, data3 = prepare_data(user_neigh_nega[i*batch_size2:(i+1)*batch_size2], u_train, e_train)
                u_input.set_value(data1, borrow=True)
                e1_input.set_value(data2, borrow=True)
                e2_input.set_value(data3, borrow=True)
                sum_right = 0
                n_train_batches = batch_size2 // batch_size #u_input.get_value(borrow=True).shape[0] // batch_size
                pair_result = []
		...
		#print "batch", (i+1), "need time : %f" % (time.time() - start)
            if (iter+1) % 5 == 0:
                Print(Params, "./result_all/result2_WB_"+"iter"+str(iter+1),[717] + nerual_num_U, [size_e] + nerual_num_E)
