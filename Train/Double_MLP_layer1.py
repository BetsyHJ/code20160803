...
def create_matrix(num_train, flag):
    ...
    return temp        
def load_data(size_e, e_train, size_u, u_train, u_c, borrow=True, dropset = 0.5):
    negative_set = int(sys.argv[1])
...
def Print(y, params_file, nerual_num_u, nerual_num_c):
    ...
    for i in range(len(params)/2):
        ...
        file.write("\n")
    file.close()
def cut_data(u_train, batch_num):
    u_keys = u_train.keys()
    u_num = len(u_keys)
    batch = u_num / batch_num
    u_trains = []
    for i in range(batch_num):
        temp = {}
        for j in range(i*batch, (i+1)*batch):
            temp[u_keys[j]] = u_train[u_keys[j]]
        u_trains.append(temp)
    return u_trains
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
   ...
   """
    ...
    u_input, e1_input, e2_input = load_data(size_e, e_train, size_u, u_trains[0], u_c)    
    nerual_num_U = [256]
    nerual_num_E = [256]
	...
	#user = e_user; size = len(user)
        for iter in range(iteration): 
            #right_each = 0
            for i in range(batch_num):
                u_input, e1_input, e2_input = load_data(size_e, e_train, size_u, u_trains[i], u_c, dropset = 0.9)
                sum_right = 0
            ...
            if (iter+1) % 50 == 0:
                Print(Params, "./result_all/result2_WB_nega"+str(sys.argv[1])+"_iter"+str(iter+1),[717] + nerual_num_U, [512] + nerual_num_E)
